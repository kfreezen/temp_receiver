...
bool SensorDB::AddReport(std::string sensor_id, time_t timestamp, double probe0_temp) {
	printf("AddReportRun()\n");
	...
	sprintf(cPOST, "timestamp=%lu&sensor_id=%s&probe0_temp=%f", timestamp, sensor_id_encoded, probe0_temp);
	curl_free(sensor_id_encoded);
